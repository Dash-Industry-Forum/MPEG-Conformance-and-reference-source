...
OSErr Validate_traf_Atom( atomOffsetEntry *aoe, void *refcon )
{
	...
	for (i = 0; i < (long)trafInfo->numTrun; i++)
	{
       ...
       // if(vg.cmaf){
	//	UInt64 sample_composition_time_offset_first = trafInfo->trunInfo[i].sample_composition_time_offset[0];
	//	if((trafInfo->baseMediaDecodeTime) != sample_composition_time_offset_first)
	//		errprint("CMAF check violated: Section 7.5.15. \"In CMAF Segments, the field baseMediaDecodeTime SHALL also equal set the first Sample composition time in the Fragment\", found %ld\n", EndianU64_BtoN(trafInfo->baseMediaDecodeTime));
	//}
	}
